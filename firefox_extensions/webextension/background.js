...
(function() {
    ...
    function convertFilenameDatabaseToCSV(filenameDatabase, settings, model) {
        ...
        const includeMetadata = settings.csvIncludeMetadata !== false;
        
        let csv = '';
        
        // Add model name as metadata rows at the top if enabled
        if (includeMetadata) {
            const now = new Date().toISOString();
            csv += `Model${separator}${model}\n`;
            csv += `Extracted${separator}${now}\n`;
            csv += `Generated by${separator}RecurTrack Extension\n`;
